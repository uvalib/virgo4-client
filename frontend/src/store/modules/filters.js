...
const filters = {
   ...
   getters: {
      ...
      globalFilter: (_state, getters, rootState) =>  {
         ...
         console.log("GOT GLOBAL FILTERS: "+JSON.stringify(out))
         return out  
      }
   },
   ...
   mutations: {
      ...
      restoreFilters(state, data) {
         console.log("restore filters for result "+data.resultsIdx)
          // wipe out any existing facet info, but init empty lists for each pool
          state.poolFacets.splice(0, state.poolFacets.length)
         ...
         // Make a skeletal facet for the filter being resored and add it to the facetes 
         // for the target pool, marking it as selected. The search will then fill in the
         // details and other available filters
         ...
         data.filters.forEach( filter => {
            ...
            let bucket = {selected: true, value: filter.value}
            let facet = facets.find(f => f.id == filter.facet_id)
            if ( facet == null ) {
               facet = {id: filter.facet_id, buckets: [bucket]}
               facets.push(facet)
            } else {
