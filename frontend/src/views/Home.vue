...
export default {
   ...
   computed: {
      ...
      ...mapFields("query", ["basic", "basicSearchScope"]),
      searchScopes() {
   ...
   methods: {
      ...
      showBookmarkTarget() {
         ...
         if (bmRestore.groupParent) {
            ...
            let parent = this.selectedResults.hits.find(
               r => r.identifier == bmRestore.groupParent
            );
            ...
            console.log("scroll to " + identifier);
            let sel = `.hit[data-identifier="${identifier}"]`;
            let tgtEle = document.body.querySelector(sel);
            this.scrollToItem(tgtEle);
            bmData.data = this.selectedResults.hits.find(
