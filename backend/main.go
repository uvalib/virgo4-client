...
func main() {
	...
	api := router.Group("/api")
	{
		...
		api.Group("/users").Use(svc.AuthMiddleware, svc.UserMiddleware)
		{
			api.GET("/users/:uid", svc.GetUser)
			api.GET("/users/:uid/illiad", svc.GetILLiadRequests)
			api.GET("/users/:uid/bills", svc.GetUserBills)
			api.GET("/users/:uid/checkouts", svc.GetUserCheckouts)
			api.GET("/users/:uid/holds", svc.GetUserHolds)
			api.POST("/users/:uid/checkouts/renew", svc.RenewCheckouts)
			api.POST("/users/:uid/preferences", svc.SavePreferences)

			api.GET("/users/:uid/searches", svc.GetUserSavedSearches)
			api.POST("/users/:uid/searches", svc.SaveSearch)
			api.DELETE("/users/:uid/searches/:token", svc.DeleteSavedSearch)
			api.PUT("/users/:uid/searches/:token", svc.UpdateSavedSearch)
			api.DELETE("/users/:uid/searches", svc.DeleteAllSavedSearches)
			api.POST("/users/:uid/searches/:token/publish", svc.PublishSavedSearch)
			api.DELETE("/users/:uid/searches/:token/publish", svc.UnpublishSavedSearch)

			api.GET("/users/:uid/bookmarks", svc.GetBookmarks)
			api.POST("/users/:uid/bookmarks/move", svc.MoveBookmarks)
			api.POST("/users/:uid/bookmarks/delete", svc.DeleteBookmarks)
			api.POST("/users/:uid/bookmarks/folders", svc.AddBookmarkFolder)
			api.DELETE("/users/:uid/bookmarks/folders/:id", svc.DeleteBookmarkFolder)
			api.POST("/users/:uid/bookmarks/folders/:id", svc.UpdateBookmarkFolder)
			api.DELETE("/users/:uid/bookmarks/folders/:id/publish", svc.UnpublishBookmarkFolder)
			api.POST("/users/:uid/bookmarks/folders/:id/publish", svc.PublishBookmarkFolder)
			api.POST("/users/:uid/bookmarks/items", svc.AddBookmark)
		}
